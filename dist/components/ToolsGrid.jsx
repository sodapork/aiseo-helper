...
import { Search, FileText, BarChart3, Target, Globe, TrendingUp, MessageSquare, Cpu } from 'lucide-react';
...
export default function ToolsGrid() {
    ...
    return (<section id="tools" className="py-20 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {tools.map(function (tool, index) { return (<motion.div key={tool.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: index * 0.1 }} viewport={{ once: true }} className="bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 group cursor-pointer" onClick={function () { return handleToolClick(tool); }}>
              <div className="p-6">
                <div className={"w-12 h-12 rounded-lg bg-gradient-to-r ".concat(tool.color, " flex items-center justify-center mb-4 group-hover:scale-110 transition-transform")}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.8 }} viewport={{ once: true }} className="text-center mt-12">
