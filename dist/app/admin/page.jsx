...
export default function AdminPage() {
    var _a = useState([]), tools = _a[0], setTools = _a[1];
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {adminSections.map(function (section, index) { return (<motion.div key={section.title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: index * 0.1 }} className="bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 group cursor-pointer">
              <Link href={section.link}>
                <div className="p-6">
                  <div className={"w-12 h-12 rounded-lg bg-gradient-to-r ".concat(section.color, " flex items-center justify-center mb-4 group-hover:scale-110 transition-transform")}>
        ...
        {/* Tools Overview */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }} className="bg-white rounded-xl shadow-sm p-6">
          ...
          {loading ? (<div className="flex items-center justify-center py-8">
              ...
              {tools.map(function (tool) { return (<div key={tool.id} className="border border-gray-200 rounded-lg p-4 hover:border-blue-300 transition-colors">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-semibold text-gray-900">{tool.name}</h3>
                    <span className={"text-xs px-2 py-1 rounded-full ".concat(tool.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800')}>
                      {tool.status}
                    </span>
                  </div>
        ...
        {/* Quick Actions */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.6 }} className="mt-8 bg-blue-50 rounded-xl p-6">
